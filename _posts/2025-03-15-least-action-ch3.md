...
**페르마의 원리(Fermat's Principle)**는 광학에서만 중요한 원리일 것으로 치부되었다. 하지만 베르누이는 놀랍게도 페르마의 원리를 역학과 연결시켰다. 어떻게 역학의 영역으로 페르마의 원리를 접목시켰을까?

**요한 베르누이(Johann Bernoulli, 1667~1748)**는 스위스 바젤(Basel) 출신의 수학자이다. 베르누이 가문은 수학과 과학 분야에서 3세대에 걸쳐 8명 이상의 저명한 학자를 배출한 것으로 유명한데, 요한 베르누이는 그중에서도 가장 영향력 있는 인물 중 하나이다.

요한 베르누이는 원래 아버지의 뜻에 따라 의학을 공부했으나, 형인 **야코프 베르누이(Jacob Bernoulli, 1655~1705)**의 영향으로 수학에 빠져들었다. 이후 **라이프니츠(Gottfried Wilhelm Leibniz)**와 깊은 학문적 교류를 나누며 미적분학의 발전에 크게 기여하였다. 그의 제자 중에는 훗날 최소 작용의 원리를 주장하게 되는 **모페르뒤(Maupertuis)**와, 역사상 가장 위대한 수학자 중 한 명인 **오일러(Euler)**가 있다.

한편, 그의 아들 **다니엘 베르누이(Daniel Bernoulli, 1700~1782)**도 뛰어난 학자였다. 다니엘 베르누이는 네덜란드 흐로닝언(Groningen)에서 태어난 스위스인으로, 수학과 의학을 모두 공부하였다. 특히 유체역학에서 **베르누이 방정식(Bernoulli's Equation)**을 발견하여 유체의 속도와 압력 사이의 관계를 밝혔다. 아이러니하게도 아버지 요한 베르누이는 아들의 업적을 질투하여, 다니엘의 저서 출판일을 자신의 것보다 늦추려 하는 등 부자 사이에 학문적 갈등이 있었다고 전해진다.

요한 베르누이는 당대 유명한 두 문제에 대해 고심했다. 바로 **최속 강하 곡선(Brachistochrone Curve)**과 **등시 곡선(Tautochrone Curve)**이다. 베르누이는 이 문제에 꽤 놀라운 해결책을 제시했다. 그건 무엇이었을까?
...
먼저, 베르누이는 **에너지 보존(Conservation of Energy)**으로부터 다음과 같은 식을 유도하였다.
...
또한, 베르누이는 **스넬의 법칙(Snell's Law)**과 굴절률 관련 식을 통해 다음과 같은 관계를 얻어내었다.
...
따라서, 베르누이는 **최속 강하 곡선(Brachistochrone Curve)**은 **사이클로이드 곡선(Cycloid Curve)**임을 찾아낸 셈이다.
...
**등시 곡선(Tautochrone Curve)**은 균일한 중력장에서 마찰없이 가장 낮은 점까지 도달하는 물체의 시간이 곡선 위의 시작점과 상관없이 동일한 곡선을 의미한다. 베르누이는 위와 유사한 방법으로 등시 곡선 또한 **사이클로이드 곡선(Cycloid Curve)**이며, 따라서 **등시 곡선과 최속 강하 곡선이 동일한 문제**임을 밝혀내었다.
...
이 글을 보고 있는 여러분은 이제 광학에서의 **페르마의 원리(Fermat's Principle)**가 역학 문제로 귀결될 수 있음을 본 것이다. **스넬의 법칙(Snell's Law)**으로 역학적 문제를 다루었다는 뜻은 곧 페르마의 원리로 역학적 문제를 다룰 수 있음을 의미한다. 광학적 문제와 역학적 문제는 서로 다른 물리 현상으로 보이지만 **최소 시간의 원리라는 공통된 수학적 원리**를 가지게 된다는 것이다.

여기까지 따라온 여러분에게 어떠한 의문이 들 수 있다. 바로 최소 시간의 원리를 역학 문제에 어떻게 명확히 정의할 수 있는지에 대해서이다. 이에 대해 설명하기에 앞서, 경로에 관한 중요한 물리량에 대해 말하지 않을 수 없다. 바로 **모페르뒤의 원리(Maupertuis's Principle)**과 **작용(Action)**이다.

---

> **챕터 이동**
>
> [이전 챕터: Ch.2 — 페르마의 원리](/series/least-action-ch2/) \| [다음 챕터: Ch.4 — 모페르뒤의 원리와 작용](/series/least-action-ch4/)
